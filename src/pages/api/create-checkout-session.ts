...
const PRICE_MAP: Record<string, string> = {
  ...
  "aftershot-zeolite":    "price_1S5d2XEBOCXQH1bcb4pLLA3z",
...
// Only build the public origin from headers (no env, no paths)
function publicOrigin(req: Request, fallback: string) {
  const host  = req.headers.get("x-forwarded-host") || req.headers.get("host");
  const proto = req.headers.get("x-forwarded-proto") || "https";
  const o = host ? `${proto}://${host}` : fallback;
  try { return new URL(o).origin; } catch { return fallback; } // ensure origin-only
}
...
export const POST: APIRoute = async ({ request, url }) => {
  try {
    ...
    // Build absolute URLs from headers only (no env/path contamination)
    const origin = publicOrigin(request, url.origin).replace(/\/$/, "");
